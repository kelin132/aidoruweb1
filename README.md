...
3. Auth System: Custom AIDORU ID + Password authentication synced with the Kelin-MD2 user collection in MongoDB.
...
1. Authentication System
   ...
   - Login using the stable AIDORU ID from `.id` and the password set with `.wpw`.
...
- Website sign-in uses the stable AIDORU ID generated by `.id` plus the password
  created with `.wpw` in a WhatsApp chat. The `.wpw` message is scheduled for deletion
  after one second when supported. Passwords are stored only as
  salted scrypt hashes in the shared `users` collection.
...
- Schema integration:
  ...
  - Website credentials: `websiteId`, `websitePasswordHash`,
    `websitePasswordUpdatedAt`, and `websiteIdCreatedAt` on the user document.
    The website ID is unique and the password is never stored in plaintext.
