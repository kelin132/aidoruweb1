...
- Ensure all API routes query and modify the MongoDB database used by Kelin-MD2.

- Schema integration:

  - Users: `{ phoneNumber, passwordHash, name, bio, coins, bank, inventory: [], guildId, starterChosen, dailyClaimedAt }`

  - Guilds: `{ name, tag, description, leaderId, members: [], level, bank }`

  - Shop Items: `{ id, name, category, price, iconUrl, description }`
